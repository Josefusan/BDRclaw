...
- [x] ISC-24: `PATCH /api/campaigns/:id { status: "active" }` enrolls all active prospects, **preserves the campaign steps**, and begins the loop processing them within one tick. *(2026-07-16: passed only AFTER the step-wipe fix — the enroll worked all along, but pre-fix the steps were gone so the loop had nothing to send. Now regression-guarded in dashboard-write-api.test.ts.)*
...
- [x] ISC-70: Campaigns can be activated AND paused from the UI; state round-trips through PATCH **without destroying the campaign's steps**. *(2026-07-16: initially marked passed on a shallow browser check (toast + pill flip) — that was a FALSE GREEN; a war-room agent's ISC-24 test caught that activation cascade-deleted all steps. Root-caused to upsertCampaign's INSERT OR REPLACE, reproduced empirically (steps 1→0), fixed to ON CONFLICT DO UPDATE (1→1), regression-tested through the real PATCH route.)*
...
- [x] ISC-85: With `BDR_DASHBOARD_PASSWORD` set, every dashboard page and `/api/*` route requires a valid session — unauthenticated page GETs 302 to `/login`, unauthenticated API calls get 401 JSON. *(2026-07-16: dashboard-auth.test.ts + live smoke: `302 → /login`, `/api/stats` 401.)*
- [x] ISC-86: `POST /api/login` with the correct password sets an HttpOnly `bdr_session` cookie and grants access; a wrong password returns 401; more than 5 failures per IP per 15 minutes returns 429. *(test: 5×401 → 429 even with correct pw, other IP unaffected; live smoke: wrong 401, right 200+cookie → /api/stats 200.)*
...
- **2026-07-16 (b)** — conjectured: "ISC-70/24 pass — I activated a campaign in the browser and it enrolled a prospect." refuted_by: a war-room agent's ISC-24 test failed where mine had 'passed'; reproduced empirically that activating a campaign runs `upsertCampaign` with the full object → `INSERT OR REPLACE` → delete-then-insert → cascade through `bdr_campaign_steps` ON DELETE CASCADE → every step gone. My browser check saw the enroll toast and the pill flip and stopped there; it never asserted the thing that matters — that the campaign still had steps to send. learned: a live check is only worth the specific state it asserts. "The button worked" is not "the feature worked"; verifying an action must assert the post-condition the feature exists FOR (a campaign that can still send), not the nearest visible signal (a toast). The regression that a passing UI hides is exactly the one no one re-checks. criterion_now: ISC-24/70 now require step-preservation, regression-tested through the real PATCH route; upsertCampaign uses ON CONFLICT DO UPDATE.
